...
module.exports = class AuthRequest {
  ...
  constructor(
    ...
    console.log('ere', Object.getPrototypeOf(Error));
    let arrErrors = [];
  ...
  async findEnduser() {
    try {
      if (this._enduser != null) {
        return { success: true, data: this._enduser };
      } else {
        this._dashConnection.connect();
  ...
  async submit() {
    try {
      ...
      if (submitted.success) {
        ...
        if (submitted.error) {
          debug(`submitted document error: ${submitted.message}`);
          return { error: true, message: submitted.message };
        } else {
          debug(`unknown submitted document error`);
          return {
            error: true,
            message: `unknown submitted document error`,
          };
        }
      }
    } catch (e) {
